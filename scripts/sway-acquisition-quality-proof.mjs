...
const candidate='e9defcf674e0fbf6ceb89e15ba45899e4d5f9621';
...
try{
 ...
 assert.deepEqual(changed,['scripts/report-acquisition-quality.mjs','scripts/sway-acquisition-quality.test.mjs','scripts/sway-telemetry.contract.test.mjs','src/server/access-control.ts','src/server/audit-log.ts','src/server/discovery-traffic.ts']);
 const accessPath=path.join(checkout,'src/server/access-control.ts'),auditPath=path.join(checkout,'src/server/audit-log.ts');
 const access=fs.readFileSync(accessPath,'utf8'),audit=fs.readFileSync(auditPath,'utf8');
 const oldAccess=await run('base-access','git',['show',base+':src/server/access-control.ts']);
 const restored=access.replace("import { runDiscoveryTraffic } from './discovery-traffic';\n",'').replace('runDiscoveryTraffic(req, next);','next();');
 assert.equal(restored.trimEnd(),oldAccess.trimEnd(),'Every existing authorization decision must remain byte-identical');
 ...
 try{
  fs.writeFileSync(accessPath,oldAccess+'\n');fs.writeFileSync(auditPath,oldAudit+'\n');
 ...
 }finally{fs.writeFileSync(accessPath,access);fs.writeFileSync(auditPath,audit);}
