...
try {
  ...
  await importFile(a.page, 'Import Mixxx file', 'Friday.csv', 'song,artist\nMixxx Friday Original,Example Performer');
  tracks = (await api(a.context, '/api/talent/library/tracks')).external.tracks;
  assert(tracks.some(track => track.title === 'Mixxx Friday Original'));
  assert(tracks.some(track => track.title === 'Friday Replacement'));
  assert.equal((await api(a.context, '/api/talent/library/sources')).sources.length, 12);
  record('The same filename imported from another named music app creates a separate source without replacing existing tracks');
  ...
  const db = await proof.query(`select count(*)::int as records
    from performer_library_tracks tracks
    join performer_library_sources sources
      on sources.performer_id = tracks.performer_id and sources.source_key = tracks.source_key
    where tracks.performer_id = $1 and sources.connection_status = 'active'`, [owner]);
  assert.equal(db.rows[0].records, 12);
  record('Server restart preserves all twelve sources and database-backed active tracks');
