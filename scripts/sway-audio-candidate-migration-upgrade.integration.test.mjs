...
async function applyMigration(database, filename) {
  const execute = (statement) => typeof database.exec === 'function'
    ? database.exec(statement)
    : database.query(statement);
  // Match the deployed migrator's transaction boundary. Current baseline
  // migrations deliberately hold table locks through preflight and backfill.
  await execute('BEGIN');
  try {
...
const migrationFiles = readdirSync(migrationsDirectory)
  ...
  .sort();
const migrationsThrough0053 = migrationFiles.filter((name) => migrationNumber(name) <= 53);
const migration0054 = migrationFiles.find((name) => migrationNumber(name) === 54);
const migration0055 = migrationFiles.find((name) => migrationNumber(name) === 55);
const migration0056 = migrationFiles.find((name) => migrationNumber(name) === 56);
const migration0057 = migrationFiles.find((name) => migrationNumber(name) === 57);
const migration0058 = migrationFiles.find((name) => migrationNumber(name) === 58);

assert.ok(migrationsThrough0053.length > 0, 'Migrations through 0053 are required.');
assert.ok(migration0054, 'Migration 0054 is required.');
assert.ok(migration0055, 'Migration 0055 is required.');
assert.ok(migration0056, 'Migration 0056 is required.');
assert.equal(migration0057, '0057_audio_provider_operation_durability.sql');
assert.equal(migration0058, '0058_audio_candidate_owner_decisions.sql');
assert.ok(migrationsThrough0053.includes('0052_performer_withdrawal_readback_schedule.sql'), 'The current deployed 0052 baseline is required.');
assert.ok(migrationsThrough0053.includes('0053_audio_collaboration_capability.sql'), 'The selective capability prerequisite is required.');
...
try {
  for (const filename of migrationsThrough0053) await applyMigration(database, filename);
  ...
  assert.equal(legacyRows.rows[0]?.can_upload_new_version, true, 'The pre-0054 fixture must exercise the valid legacy upload permission.');
  const originalVersionBeforeUpgrade = await database.query(
    `select to_jsonb(version) as preserved from audio_project_asset_versions version where id = $1::uuid`,
    [sourceVersionId]
  );
  const projectAuthorityBeforeUpgrade = await database.query(
    `select to_jsonb(authority) as preserved from audio_project_access_grants authority where id = $1::uuid`,
    [projectAccessGrantId]
  );
  assert.equal(originalVersionBeforeUpgrade.rows.length, 1);
  assert.equal(projectAuthorityBeforeUpgrade.rows.length, 1);

  await applyMigration(database, migration0054);
  ...
  await database.query(
    ...
    [
      ...
      JSON.stringify({ proof: 'candidate_grant_before_0056' }),
      'f'.repeat(64)
    ]
  );
  await database.query(
  ...
  await database.query(
    `insert into audio_upload_sessions (
       ...
       $10, 'candidate-before-0056.wav', 'audio/wav', 844,
    ...
    [
      ...
      `candidate-upload-before-0056-${candidateUploadSessionId}`,
  ...
  await applyMigration(database, migration0055);
  await database.query(
    ...
    [
      ...
      projectId,
      collaboratorUserId,
      `legacy/${projectId}/orphan-before-0056/candidate.wav`,
  ...
  await database.query(
    ...
    [
      ...
      ownerUserId,
      uploadSessionId,
      `legacy/${projectId}/invalid-null-provider-before-0056/source.wav`,
      'Fixture must be rejected before any 0056 DDL is applied.'
    ]
  );
  ...
  assert.ok(upgradedGrant.revoked_at, 'Legacy upload authority must be explicitly revoked during 0054.');
  ...
  const activeAfter0055 = await database.query(
    `select id from audio_file_access_grants where id = $1::uuid and revoked_at is null`,
    [legacyGrantId]
  );
  assert.equal(
    activeAfter0055.rows.length,
    0,
    'Legacy upload authority must not be silently converted into active review access.'
  );

  await assert.rejects(
    applyMigration(database, migration0056),
    (error) => /cleanup receipt preflight failed/i.test(
      String(error?.cause?.message || error?.message || error)
    )
  );
  const rejectedMigrationColumns = await database.query(
    `select column_name from information_schema.columns
     where table_schema = 'public' and table_name = 'audio_file_access_grants'
       and column_name = 'max_candidate_bytes'`
  );
  assert.equal(rejectedMigrationColumns.rows.length, 0, 'A failed populated preflight must stop before the first 0056 schema change.');
  await database.query(
    `delete from audio_object_cleanup_receipts where id = $1::uuid`,
    [invalidCleanupReceiptId]
  );
  await applyMigration(database, migration0056);
  ...
  assert.equal(finalRows.rows.length, 1, 'The sanitized legacy grant must remain present after 0056.');
  ...
  assert.equal(migratedCandidateRows.rows.length, 1, 'The valid pre-0056 candidate grant must survive as audit evidence.');
  ...
  assert.ok(migratedCandidateGrant.revoked_at, 'A pre-ceiling candidate grant must be explicitly revoked by 0056.');
  ...
  assert.equal(migratedReceipts.rows.length, 2, 'Both valid pre-0056 cleanup receipt shapes must survive 0056.');
  ...
  // Complete the actual current migration chain only after proving the
  // populated pre-hardening upgrade and its fail-before-DDL preflight.
  await applyMigration(database, migration0057);
  await applyMigration(database, migration0058);
  const originalVersionAfterUpgrade = await database.query(
    `select to_jsonb(version) as preserved from audio_project_asset_versions version where id = $1::uuid`,
    [sourceVersionId]
  );
  const projectAuthorityAfterUpgrade = await database.query(
    `select to_jsonb(authority) as preserved from audio_project_access_grants authority where id = $1::uuid`,
    [projectAccessGrantId]
  );
  assert.deepEqual(originalVersionAfterUpgrade.rows, originalVersionBeforeUpgrade.rows,
    'The completed 0058 upgrade must preserve every original master identity, verification and provenance field.');
  assert.deepEqual(projectAuthorityAfterUpgrade.rows, projectAuthorityBeforeUpgrade.rows,
    'The completed 0058 upgrade must preserve existing project access authority.');
  const receiptsAfterProviderUpgrade = await database.query(
    `select id, upload_session_id, storage_provider, storage_bucket, storage_key,
            provider_upload_id, cleanup_reason, cleanup_status
     from audio_object_cleanup_receipts
     where id in ($1::uuid, $2::uuid)
  ...
  assert.deepEqual(receiptsAfterProviderUpgrade.rows, migratedReceipts.rows,
    'Provider recovery and owner decisions must retain both populated cleanup receipt shapes.');
  const laterTables = await database.query(
    `select
       (select count(*)::int from audio_provider_operations) as operations,
       (select count(*)::int from audio_provider_operation_attempts) as attempts,
       (select count(*)::int from audio_provider_operation_resolutions) as resolutions,
       (select count(*)::int from audio_candidate_owner_decisions) as decisions`
  );
  assert.deepEqual(laterTables.rows[0], { operations: 0, attempts: 0, resolutions: 0, decisions: 0 },
    'The upgrade must create the current recovery/decision tables without inventing provider intent or owner acceptance.');
  await assert.rejects(
    database.query(
      `select sway_require_active_collaborator_revision_grant(
         $1::uuid, $2::uuid, $3::uuid, $4::uuid, $5::uuid
    ...
    (error) => /active exact-file upload grant/i.test(String(error?.message || error?.cause?.message || error)),
    'Later recovery and acceptance migrations must not restore the revoked pre-ceiling upload grant.'
  );

  console.log(
    `Audio candidate migration upgrade integration passed on ${proofKind}: a valid legacy upload-enabled `
      + 'selected-file grant from the populated current baseline through 0052 plus capability 0053 is explicitly revoked and sanitized by 0054/0055, then remains schema-valid '
      + 'through 0056 as an inactive review-only record with max_candidate_bytes null; a valid pre-ceiling '
      + 'candidate grant is capped and revoked, cannot authorize intake, and both orphan and exact-session '
      + 'cleanup receipts survive the new 0056 constraints; invalid populated identity is rejected before '
      + 'DDL and new cleanup evidence must begin pending at attempt one. Final 0057/0058 migrations preserve the exact source, project authority and cleanup receipts without creating provider intent or owner decisions.'
  );
} finally {
  if (strictRealPostgresProof) {
    await database.end();
