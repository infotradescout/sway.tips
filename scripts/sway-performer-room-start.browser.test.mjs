...
const cases=[
  ...
  ['account-change-during-create',async page=>{await select(page,'room-A');await start(page);await page.evaluate(()=>{window.__profile={...window.__profile,owner_user_id:'owner-B',performer_id:'performer-B'};dispatchEvent(new Event('sway:performer-profile-updated'));});await wait(page,"window.__dash.performerProfile?.owner_user_id==='owner-B'");await complete(page);assert.equal(await selected(page),'room-A');assert.equal(await page.evaluate(()=>window.__applied.length),0);}],
