...
const embeddedMode = process.argv.includes('--embedded-postgres');
...
if (embeddedMode && (
  ...
  throw new Error(
    'Embedded candidate grant behavior proof refuses external database targets or a strict real-PostgreSQL proof requirement.'
  );
}
if (!embeddedMode && !databaseUrl) {
  throw new Error(
    'SWAY_REAL_POSTGRES_PROOF_DATABASE_URL is required for the strict real-PostgreSQL candidate grant concurrency proof.'
  );
}
if (!embeddedMode && process.env.SWAY_ALLOW_DISPOSABLE_DATABASE_RESET !== 'true') {
  throw new Error(
...
async function createEmbeddedProofDatabase() {
  ...
  try {
    ...
    return { database, migrationCount: migrationFiles.length };
  } catch (error) {
...
async function raceBehindIdempotencyBarrier({
  ...
  if (embeddedMode) {
    // PGlite shares one backend. These sequential calls prove persisted
    // idempotency/conflict behavior only, never lock ordering or concurrency.
    const results = [];
    for (const call of calls) {
      try {
        results.push({ status: 'fulfilled', value: await within(call(), `Embedded serial behavior: ${label}`) });
      } catch (reason) {
...
async function main() {
  const embedded = embeddedMode ? await createEmbeddedProofDatabase() : null;
  const attestation = embeddedMode ? null : await attestResetAndMigrate();
  const objectRoot = mkdtempSync(join(tmpdir(), 'sway-candidate-grant-concurrency-'));
  const embeddedQuery = embedded ? { query: (text, values) => embedded.database.query(text, values) } : null;
  const poolA = embeddedQuery || createProofPool('sway_candidate_grant_racer_a');
  const poolB = embeddedQuery || createProofPool('sway_candidate_grant_racer_b');
  const dbA = embedded ? drizzlePglite(embedded.database, { schema }) : drizzle(poolA, { schema });
  const dbB = embedded ? dbA : drizzle(poolB, { schema });
  ...
  try {
    ...
    if (!embeddedMode) {
      ...
      backendPids.push(backendA.rows[0]?.backend_pid, backendB.rows[0]?.backend_pid);
    ...
    console.log(JSON.stringify({
      proof: embeddedMode
        ? 'embedded PGlite serial candidate-grant behavior; standalone concurrency NOT verified'
        : 'attested standalone PostgreSQL candidate-grant concurrency',
      ...(embeddedMode ? {
        execution: 'sequential calls on one disposable in-memory backend',
        standaloneConcurrencyVerified: false,
        migrationCount: embedded.migrationCount
      } : {
      ...
      [embeddedMode ? 'differentIntentSequence' : 'differentIntentRace']: {
        created: 1,
      ...
      [embeddedMode ? 'exactIntentSequence' : 'exactIntentRace']: {
        created: 1,
        reused: 1,
        rowsForGrantorKeyHash: exactRows.length,
        convergedGrantId: exactValues[0].grant.id
      },
      [embeddedMode ? 'differentKeyExactIntentSequence' : 'differentKeyExactIntentRace']: {
        created: 1,
      ...
      [embeddedMode ? 'differentKeyConflictingIntentSequence' : 'differentKeyConflictingIntentRace']: {
        created: 1,
      ...
      [embeddedMode ? 'exactCandidateInitiationSequence' : 'exactCandidateInitiationRace']: {
    ...
    if (embedded) await embedded.database.close();
    else await Promise.allSettled([poolA.end(), poolB.end()]);
    rmSync(objectRoot, { recursive: true, force: true });
  }
}

main().catch((error) => {
  console.error(embeddedMode
    ? 'Embedded serial audio candidate grant behavior proof failed:'
    : 'Audio candidate grant concurrency integration proof failed:');
