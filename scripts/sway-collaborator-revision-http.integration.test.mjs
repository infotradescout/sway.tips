...
async function startSwayServer(input) {
  ...
  const child = spawn(process.execPath, ['--import', 'tsx', 'server.ts'], {
    ...
    env: {
      ...
      SWAY_AUDIO_STORAGE_PROVIDER: input.storageProvider ?? 'local_private_fs',
...
let storageUnavailableServer = null;
...
try {
  ...
  const changedAccountRequest = await request(enabledServer, '/api/talent/audio/files/shared-with-me', {
    token: collaboratorSession.token, headers: { 'X-Sway-Expected-Account-Id': ownerUserId }
  });
  assertStatus(changedAccountRequest, 409, 'Changed inbox account context', enabledServer);
  assert.equal(changedAccountRequest.body.code, 'account_context_changed');
  const matchedAccountRequest = await request(enabledServer, '/api/talent/audio/files/shared-with-me', {
    token: collaboratorSession.token, headers: { 'X-Sway-Expected-Account-Id': collaboratorUserId }
  });
  assertStatus(matchedAccountRequest, 200, 'Matched inbox account context', enabledServer);
  ...
  for (const [label, response] of [
    ...
    assertExactKeys(response.body, ['error'], 'Outsider ' + label + ' candidate DTO');
  }
  assert.equal(
  ...
  // Access revocation must still commit when the server has no storage client.
  // Keep a real unsealed multipart upload on a separate connection so that the
  // receipt must name its actual provider identity and cannot claim deletion.
  const [offlineMemberOneUserId, offlineMemberTwoUserId] = [ownerUserId, outsiderUserId].sort();
  const [offlineConnection] = await db.insert(audioFileConnections).values({
    memberOneUserId: offlineMemberOneUserId,
    memberTwoUserId: offlineMemberTwoUserId,
    createdByUserId: ownerUserId,
    createdFromPurpose: 'request_files'
  }).returning();
  const offlineGrantResponse = await request(
    enabledServer,
    '/api/talent/audio/pairing/connections/' + offlineConnection.id + '/candidate-revision-grants',
    {
      method: 'POST',
      token: ownerSession.token,
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        versionId: sourceVersion.id,
        idempotencyKey: 'offline-storage-connection-grant',
        maxCandidateBytes: MAX_CANDIDATE_BYTES,
        expiresInHours: 24
      })
    }
  );
  assertStatus(offlineGrantResponse, 201, 'Offline-storage fixture grant', enabledServer);
  const offlineGrantId = String(offlineGrantResponse.body.grant.id);
  const offlineCandidateBody = wavFixture('pending candidate while storage is unavailable');
  const offlineInitiationResponse = await request(
    enabledServer,
    '/api/talent/audio/file-grants/' + offlineGrantId + '/candidate-uploads',
    {
      method: 'POST',
      token: outsiderSession.token,
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        originalFilename: 'offline-pending-candidate.wav',
        mimeType: 'audio/wav',
        expectedByteSize: offlineCandidateBody.byteLength,
        expectedSha256: sha256(offlineCandidateBody),
        idempotencyKey: 'offline-storage-pending-upload'
      })
    }
  );
  assertStatus(offlineInitiationResponse, 201, 'Offline-storage fixture initiation', enabledServer);
  const offlineUploadSessionId = String(offlineInitiationResponse.body.uploadSession.id);
  const offlinePartResponse = await request(
    enabledServer,
    '/api/talent/audio/file-grants/' + offlineGrantId
      + '/candidate-uploads/' + offlineUploadSessionId + '/parts/1',
    {
      method: 'PUT',
      token: outsiderSession.token,
      headers: { 'content-type': 'application/octet-stream' },
      body: offlineCandidateBody
    }
  );
  assertStatus(offlinePartResponse, 200, 'Offline-storage fixture part', enabledServer);
  const [offlineUpload] = await db.select().from(audioUploadSessions)
    .where(eq(audioUploadSessions.id, offlineUploadSessionId));
  assert.equal(offlineUpload.uploadPurpose, 'collaborator_revision');
  assert.equal(offlineUpload.uploadStatus, 'uploading');
  assert.equal(offlineUpload.collaboratorFileGrantId, offlineGrantId);
  assert.ok(offlineUpload.providerUploadId);
  assert.equal(await countUploadParts(db, offlineUploadSessionId), 1);
  assert.equal((await db.select().from(audioObjectCleanupReceipts)
    .where(eq(audioObjectCleanupReceipts.uploadSessionId, offlineUploadSessionId))).length, 0);

  // Stop the configured recovery worker before measuring the offline path.
  await stopSwayServer(enabledServer);
  enabledServer = null;
  const storageBeforeOfflineRevocation = snapshotStorageTree(objectRoot);
  storageUnavailableServer = await startSwayServer({
    databaseUrl: embeddedPostgres.databaseUrl,
    objectRoot,
    candidateUploadsEnabled: true,
    storageProvider: ''
  });
  const offlineRevocation = await request(
    storageUnavailableServer,
    '/api/talent/audio/pairing/connections/' + offlineConnection.id + '/revoke',
    {
      method: 'POST',
      token: ownerSession.token,
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ reason: 'HTTP proof: revoke access while private storage is unavailable.' })
    }
  );
  assertStatus(offlineRevocation, 202, 'DB-only connection revocation must respond', storageUnavailableServer);
  assert.equal(offlineRevocation.body.candidateUploadCleanup.state, 'retry_pending');
  assertRecursivelyExcludesInternals(offlineRevocation.body, 'DB-only connection revocation DTO');
  const [revokedOfflineConnection] = await db.select().from(audioFileConnections)
    .where(eq(audioFileConnections.id, offlineConnection.id));
  assert.ok(revokedOfflineConnection.revokedAt, 'The DB-only route must durably revoke the connection.');
  const revokedOfflineGrants = await db.select().from(audioFileAccessGrants)
    .where(eq(audioFileAccessGrants.connectionId, offlineConnection.id));
  assert.ok(revokedOfflineGrants.length > 0);
  assert.ok(revokedOfflineGrants.every(grant => grant.revokedAt),
    'The DB-only route must durably revoke every grant on the connection.');
  const offlineReceipts = await db.select().from(audioObjectCleanupReceipts)
    .where(eq(audioObjectCleanupReceipts.uploadSessionId, offlineUploadSessionId));
  assert.equal(offlineReceipts.length, 1, 'Pending upload cleanup must have one durable receipt.');
  const [offlineReceipt] = offlineReceipts;
  assert.deepEqual({
    projectId: offlineReceipt.projectId,
    uploadSessionId: offlineReceipt.uploadSessionId,
    storageProvider: offlineReceipt.storageProvider,
    storageBucket: offlineReceipt.storageBucket,
    storageKey: offlineReceipt.storageKey,
    providerUploadId: offlineReceipt.providerUploadId
  }, {
    projectId: offlineUpload.projectId,
    uploadSessionId: offlineUpload.id,
    storageProvider: offlineUpload.storageProvider,
    storageBucket: offlineUpload.storageBucket,
    storageKey: offlineUpload.storageKey,
    providerUploadId: offlineUpload.providerUploadId
  }, 'Offline cleanup must retain the actual pending upload identity.');
  assert.equal(offlineReceipt.cleanupReason, 'candidate_connection_revoked');
  assert.equal(offlineReceipt.cleanupStatus, 'pending');
  assert.equal(offlineReceipt.completedAt, null);
  const [stillPendingOfflineUpload] = await db.select().from(audioUploadSessions)
    .where(eq(audioUploadSessions.id, offlineUploadSessionId));
  assert.equal(stillPendingOfflineUpload.uploadStatus, 'uploading',
    'Offline revocation must not claim provider cleanup completed.');
  assert.deepEqual(snapshotStorageTree(objectRoot), storageBeforeOfflineRevocation,
    'A DB-only revoke must leave private provider bytes unchanged while cleanup is pending.');
  await stopSwayServer(storageUnavailableServer);
  storageUnavailableServer = null;
  enabledServer = await startSwayServer({
    databaseUrl: embeddedPostgres.databaseUrl,
    objectRoot,
    candidateUploadsEnabled: true
  ...
  assert.match(collaboratorOpenAfterRevocation.body.error, /unavailable or your access has ended/);
  ...
  const rangedCandidate = await request(enabledServer, candidateContentPath, {
    token: ownerSession.token, headers: { Range: 'bytes=4-31' }
  });
  assertStatus(rangedCandidate, 206, 'Owner candidate byte range', enabledServer);
  assert.deepEqual(rangedCandidate.bytes, candidateBody.subarray(4, 32));
  assert.equal(rangedCandidate.headers.get('content-range'), `bytes 4-31/${candidateBody.length}`);
  assert.equal(rangedCandidate.headers.get('cache-control'), 'private, no-store');
  ...
  assert.equal(providerFailureResponse.body.error, 'Audio listening is temporarily unavailable.');
  ...
  console.log(
    'Sway Wave 5A/5B real Express HTTP trust-boundary proof passed: flag-off, anonymous, '
      ...
      + 'DB-only connection revocation returned 202, revoked grants, and preserved pending provider bytes; '
