...
if (adminOpsCompatSource) {
  for (const required of [
    ...
    if (!adminOpsCompatSource.includes(required)) {
      failures.push(`${adminOpsCompatFile} missing required Phase 10 extracted fragment token: ${required}`);
    }
  }
}
...
if (adminAppSource) {
  for (const required of [
    ...
    if (!adminAppSource.includes(required)) {
      failures.push(`${adminAppFile} must consume Phase 10 extracted fragment token: ${required}`);
    }
  }
}
...
if (failures.length) {
  console.error('Phase 10 admin ops fragment extraction contract failed:');
  failures.forEach((failure) => console.error(`- ${failure}`));
  process.exit(1);
}

console.log('Phase 10 admin ops fragment extraction contract passed.');
