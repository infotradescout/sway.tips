...
export const sourceSha='0c35a0f1d4e17ac22d2c83c84ab8765453eaa402';
export const downloadSourceSha='0c35a0f1d4e17ac22d2c83c84ab8765453eaa402';
...
if(process.env.GRINDZONE_PHONE_ENABLED==='true'){
  ...
  if(live?.commit===process.env.RENDER_GIT_COMMIT&&live?.status==='ok')run(target,process.execPath,['tools/verify-phone-connection.mjs',hostRoot,'live',evidence]);
  ...
  for(const mode of ['local','live']){
    const report=path.join(evidence,mode+'-phone.json');if(!existsSync(report))continue;
