...
export const sourceSha='252f9281611c8132e76e230818596fc50ae2db51';
export const downloadSourceSha='252f9281611c8132e76e230818596fc50ae2db51';
...
async function locationPreview(evidence,mode){
  const report=JSON.parse(readFileSync(path.join(evidence,mode+'-locations.json'),'utf8'));
  if(report.passed!==true||report.source!==downloadSourceSha||report.automaticExactKillGpsVerified!==false)throw Error('Location visual evidence is incomplete');
  const {default:sharp}=await import('sharp');
  const bytes=await sharp(readFileSync(path.join(evidence,mode+'-locations.png'))).resize({width:390,withoutEnlargement:true}).jpeg({quality:55}).toBuffer();
  if(bytes.length>65536)throw Error('Location visual receipt exceeds its limit');
  const encoded=bytes.toString('base64'),sha256=createHash('sha256').update(bytes).digest('hex'),total=Math.ceil(encoded.length/4096);
  for(let part=0;part<total;part++)console.log('GRINDZONE_LOCATION_PREVIEW '+JSON.stringify({source:downloadSourceSha,mode,mime:'image/jpeg',sha256,part,total,data:encoded.slice(part*4096,(part+1)*4096)}));
}
/** The same bounded preview path for the disposable herd workflow; never reads user media. */
async function herdPreview(evidence,mode){
  const report=JSON.parse(readFileSync(path.join(evidence,mode+'-herds.json'),'utf8'));
  if(report.passed!==true||report.source!==downloadSourceSha||report.realPlayerSaveVerified!==false)throw Error('Herd visual evidence is incomplete');
  const {default:sharp}=await import('sharp');
  const bytes=await sharp(readFileSync(path.join(evidence,mode+'-herds.png'))).resize({width:390,withoutEnlargement:true}).jpeg({quality:55}).toBuffer();
  if(bytes.length>65536)throw Error('Herd visual receipt exceeds its limit');
  const encoded=bytes.toString('base64'),sha256=createHash('sha256').update(bytes).digest('hex'),total=Math.ceil(encoded.length/4096);
  for(let part=0;part<total;part++)console.log('GRINDZONE_HERD_PREVIEW '+JSON.stringify({source:downloadSourceSha,mode,mime:'image/jpeg',sha256,part,total,data:encoded.slice(part*4096,(part+1)*4096)}));
}
/** Public guest UI with generated player reports, never an account or private source screenshot. */
async function browserPlayPreview(evidence,mode){
  const report=JSON.parse(readFileSync(path.join(evidence,mode+'-browser-play.json'),'utf8'));
  if(report.passed!==true||report.source!==downloadSourceSha||report.pcProcessesCreated!==0||report.accountSyncVerified!==false)throw Error('No-PC browser evidence is incomplete');
  const {default:sharp}=await import('sharp');
  const input=readFileSync(path.join(evidence,mode+'-browser-play.png')),meta=await sharp(input).metadata();
  const bytes=await sharp(input).extract({left:0,top:0,width:meta.width,height:Math.min(meta.height,844)}).resize({width:320,withoutEnlargement:true}).jpeg({quality:45}).toBuffer();
  if(bytes.length>65536)throw Error('Browser preview exceeds its limit');
  const encoded=bytes.toString('base64'),sha256=createHash('sha256').update(bytes).digest('hex'),total=Math.ceil(encoded.length/4096);
  for(let part=0;part<total;part++)console.log('GRINDZONE_BROWSER_PLAY_PREVIEW '+JSON.stringify({source:downloadSourceSha,mode,mime:'image/jpeg',sha256,part,total,data:encoded.slice(part*4096,(part+1)*4096)}));
}
if(process.env.GRINDZONE_PHONE_ENABLED==='true'){
  ...
  run(target,process.execPath,['tools/build-herd-reference.mjs']);
  if(relay!==target)run(relay,process.execPath,['tools/build-herd-reference.mjs']);
  ...
  await herdPreview(evidence,'local');
  run(target,process.execPath,['tools/verify-hunt-locations.mjs',hostRoot,'local',evidence]);
  await locationPreview(evidence,'local');
  run(target,process.execPath,['tools/verify-save-data.mjs',hostRoot,'local',evidence]);
  ...
  if(live?.commit===process.env.RENDER_GIT_COMMIT&&live?.status==='ok'){
    ...
    await herdPreview(evidence,'live');
    run(target,process.execPath,['tools/verify-hunt-locations.mjs',hostRoot,'live',evidence]);
    await locationPreview(evidence,'live');
    run(target,process.execPath,['tools/verify-save-data.mjs',hostRoot,'live',evidence]);
  ...
  for(const mode of ['local','live'])for(const kind of ['phone','zones','discovery','cache','studio','save-data','locations','herds','browser-play']){
    ...
    if(!existsSync(report)){if(mode==='local')throw Error('Required local acceptance report missing: '+kind);continue;}
