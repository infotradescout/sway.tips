...
const candidate='21f1dbd4a7b1cf9b6c45b9ba538f4de5bc538092',base='8ac9e03c242212f43fdfe882bbf70ad0a02cece8';
...
try{
 ...
 assert.deepEqual((await run('bounded-delta','git',['diff','--name-only',base,candidate])).split('\n').sort(),['scripts/sway-discovery-entry.browser.test.mjs','scripts/sway-telemetry.contract.test.mjs','src/shells/SwayInstallPrompt.tsx','src/shells/frictionClient.ts']);
 await run('npm-ci','npm',['ci','--include=dev','--no-audit','--no-fund']);
 await run('telemetry-and-browser',process.execPath,['scripts/sway-telemetry.contract.test.mjs']);
 ...
 await run('negative-baseline',process.execPath,['scripts/sway-discovery-entry.browser.test.mjs'],baseline,1);
 const old=JSON.parse(fs.readFileSync(path.join(baseline,'tmp/public-entry-qa/discovery-entry/results.json'),'utf8')).results;
 assert.equal(old.length,8);assert(old.some(r=>r.storage==='getters-denied'&&!r.passed&&r.pageErrors.length));assert(old.some(r=>r.storage==='available'&&!r.passed));report.baselineFailedCases=old.filter(r=>!r.passed).length;report.baselineCases=old;
