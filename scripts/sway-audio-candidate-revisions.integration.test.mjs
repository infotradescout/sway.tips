...
try {
  ...
  assert.deepEqual(await collaboration.listCandidateRequestProjectIds({ userId: ownerUserId }), [],
    'Global upload availability must not advertise creator requests before enrollment.');
  await recordCapability('granted', 'initial');
  assert.deepEqual(await collaboration.listCandidateRequestProjectIds({ userId: ownerUserId }), [project.id]);
  assert.deepEqual(await collaboration.listCandidateRequestProjectIds({ userId: collaboratorUserId }), [],
    'Receiving an upload grant must not advertise project management authority.');
  ...
  const preflightInput = {
    grantId: sealedGrant.grant.id, actorUserId: collaboratorUserId,
    originalFilename: 'sealed-candidate.wav', mimeType: 'audio/wav', expectedByteSize: candidateBody.byteLength
  };
  const beforePreflight = await db.execute(sql`select
    (select count(*) from audio_upload_sessions)::int as sessions,
    (select count(*) from audio_provider_operations)::int as operations`);
  assert.deepEqual(await publishing.preflightCollaboratorRevisionUpload(preflightInput), {
    ok: true, maxCandidateBytes: MAX_CANDIDATE_BYTES
  });
  await assert.rejects(publishing.preflightCollaboratorRevisionUpload({ ...preflightInput, actorUserId: outsiderUserId }),
    (error) => error.status === 403);
  await assert.rejects(publishing.preflightCollaboratorRevisionUpload({ ...preflightInput, expectedByteSize: MAX_CANDIDATE_BYTES + 1 }),
    (error) => error.status === 413);
  await assert.rejects(publishing.preflightCollaboratorRevisionUpload({ ...preflightInput, mimeType: 'image/png' }));
  assert.deepEqual((await db.execute(sql`select
    (select count(*) from audio_upload_sessions)::int as sessions,
    (select count(*) from audio_provider_operations)::int as operations`)).rows, beforePreflight.rows,
    'Preflight must never create an upload or reserve a provider operation.');
  ...
  assert.equal((await publishing.preflightCollaboratorRevisionUpload(preflightInput)).ok, true,
    'Matching request retry must not reserve or double count storage.');
  ...
  await assert.rejects(
    ...
    `),
    (error) => errorChainMatches(error, /require an exact immutable owner acceptance/i)
  );
  await assert.rejects(
  ...
  for (const [rangeHeader, expected] of [
    ['bytes=4-31', candidateBody.subarray(4, 32)],
    ['bytes=-17', candidateBody.subarray(candidateBody.length - 17)]
  ]) {
    const ranged = await collaboration.openCandidateRevision({
      grantId: sealedGrant.grant.id, candidateId: candidate.id, userId: collaboratorUserId, rangeHeader
    });
    assert.equal(ranged.byteSize, expected.length);
    assert.deepEqual(await streamToBuffer(ranged.stream), expected);
  }
