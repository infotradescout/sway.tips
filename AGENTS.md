...
- Governing Sway.DIO economics (summary): `docs/SWAY_PRODUCT_STRUCTURE.md` — zero-take, listener-directed streaming; per-listener monthly creator pool; Sway keeps $0 from the streaming pool; no fixed per-stream penny promise; masters retained; splits paid at source. A separate DIO economic-model document is a later Self-Production artifact, not required for this lane.
