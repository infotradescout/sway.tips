...
app.get("/api/release-health", async (_req, res) => {
  ...
  if (databaseConfigured) {
    ...
    try {
      ...
      const migrationRows = await probePool.query<{ hash: string }>(
        'SELECT hash FROM drizzle.__drizzle_migrations ORDER BY id ASC'
      );
      appliedHashes = migrationRows.rows
        .map((row) => row.hash)
  ...
  res.status(report.releaseActive ? 200 : 503).json(report);
});
