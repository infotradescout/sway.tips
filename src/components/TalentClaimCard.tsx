  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/talent/claim/accept', {
        ...
        body: JSON.stringify({ token: code, email, phone, password, confirmPassword, termsAccepted })
      });
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100">
      <div className="mx-auto max-w-xl rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(34,211,238,0.16),_transparent_36%),linear-gradient(180deg,_rgba(15,23,42,0.98),_rgba(2,6,23,1))] p-6 shadow-2xl sm:p-8">
        ...
        <p className="mt-5 text-[10px] font-black uppercase tracking-[0.28em] text-cyan-300">One-time claim code</p>
        <h1 className="mt-2 font-display text-3xl font-black text-white">Claim your account</h1>
        ...
        <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
          ...
          <label className="block space-y-1.5">
            <span className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Phone</span>
            <input
              type="tel"
              autoComplete="tel"
              value={phone}
          ...
          >
            ...
            {status === 'submitting' ? 'Finishing setup...' : 'Claim account'}
          </button>
        </form>
      </div>
    </div>
