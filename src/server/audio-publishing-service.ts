...
import {
  assertAudioVersionCandidateModeration,
  listRestrictedAudioVersionIds,
  lockAudioVersionCandidateModeration,
  openAudioVersionWithCandidateModeration
} from './audio-candidate-moderation';
import {
...
export function createAudioPublishingService(config: {
  ...
  async function validatePlayableAudioOriginal(version: {
    ...
    storageKey: string;
  }, purpose: 'private_candidate' | 'release_master' = 'private_candidate', executor: Pick<SwayDb, 'execute'> = db) {
    ...
    const openExactOriginal = async () => {
      const opened = version.id
        ? await openAudioVersionWithCandidateModeration(executor, version.id, () => store.openOriginal(identity))
        : await store.openOriginal(identity);
    ...
    try {
      ...
      if (version.id) await assertAudioVersionCandidateModeration(executor, version.id);
      return {
        container,
  ...
  async function validateReleasePackageAsset(input: {
    ...
    await assertAudioVersionCandidateModeration(executor, version.id);
    ...
    const openExactOriginal = async () => {
      const opened = await openAudioVersionWithCandidateModeration(executor, version.id, () => store.openOriginal(identity));
      if (opened.byteSize !== version.byteSize) {
        opened.stream.destroy();
    ...
    if (roleKind === 'master') {
      const validation = await validatePlayableAudioOriginal(version, 'release_master', executor);
      return {
  ...
  async function requireCandidateUploadScope(
    tx: AudioProviderOperationTransaction,
    input: { grantId: string; actorUserId: string; mimeType: string; expectedByteSize: number }
  ) {
  ...
  async function preflightCollaboratorRevisionUpload(input: {
    grantId: string; actorUserId: string; originalFilename: string; mimeType: string; expectedByteSize: number;
  }) {
    if (!collaboratorRevisionUploadsEnabled) {
      throw Object.assign(new Error('Private candidate uploads are disabled.'), { status: 503, code: 'candidate_uploads_disabled' });
    }
    const filename = input.originalFilename.trim();
    if (!filename || filename.length > 255 || !Number.isSafeInteger(input.expectedByteSize) || input.expectedByteSize <= 0) {
      throw Object.assign(new Error('A filename and positive file size are required.'), { status: 422 });
    }
    return db.transaction(async (tx) => {
      const { scope, uploadType } = await requireCandidateUploadScope(tx, input);
      const [existing] = await tx.select().from(audioUploadSessions)
        .where(eq(audioUploadSessions.collaboratorFileGrantId, scope.grantId)).limit(1);
      const [operation] = await tx.select().from(audioProviderOperations).where(and(
      ...
      if (existing || operation) {
        ...
        const matches = existing
          ? existing.originalFilename === filename && existing.expectedByteSize === input.expectedByteSize && existing.expectedMimeType === uploadType.mimeType
          : payload?.originalFilename === filename && payload?.expectedByteSize === input.expectedByteSize && payload?.mimeType === uploadType.mimeType;
        if (!matches) throw Object.assign(new Error('This request already belongs to another candidate file.'), { status: 409, code: 'candidate_upload_intent_conflict' });
      } else {
        const usage = await loadAudioStorageUsage(tx, storagePolicy, { performerId: scope.performerId });
        assertAudioWorkingObjectAvailable(usage);
        assertAudioStorageReservationAvailable(usage, input.expectedByteSize);
      }
      return { ok: true as const, maxCandidateBytes: scope.maxCandidateBytes };
    });
  }
  ...
  async function createShareGrant(input: {
    ...
    await assertAudioVersionCandidateModeration(db, version.id);
  ...
  async function downloadSharedOriginal(input: {
    ...
    const object = await openAudioVersionWithCandidateModeration(db, version.id, () => store.openOriginal({
      storageProvider: parseAudioStorageProvider(version.storageProvider),
      storageBucket: version.storageBucket,
      storageKey: version.storageKey
    }));
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        await assertAudioVersionCandidateModeration(tx, version.id);
      });
    } catch (error) {
  ...
  async function listReleaseWorkspace(
    ...
    executor: Pick<SwayDb, 'select' | 'execute'> = db
  ) {
    ...
    const restrictedVersionIds = await listRestrictedAudioVersionIds(executor, [
      ...masterRows.map(row => row.versionId),
      ...recordings.flatMap(recording => recording.masterAssetVersionId ? [recording.masterAssetVersionId] : [])
    ]);
    ...
    const masters = latestAssets.filter((row) => row.mimeType.startsWith('audio/') && !restrictedVersionIds.has(row.versionId));
    ...
    return {
      ...
      releases: releases.map((release) => ({
        ...
        readiness: (() => {
          ...
          const moderationIssues = recordings.filter(recording => recording.releaseId === release.id
            && recording.masterAssetVersionId && restrictedVersionIds.has(recording.masterAssetVersionId))
            .map(recording => `${recording.title}: candidate moderation must be cleared before this master can be used.`);
          return { ...readiness, ready: readiness.ready && !moderationIssues.length,
            issues: [...readiness.issues, ...moderationIssues], moderationIssues };
        })()
      }))
    };
  }
  ...
  async function createReleaseDraft(input: {
    ...
    return db.transaction(async (tx) => {
      await lockAudioVersionCandidateModeration(tx, master.id);
  ...
  async function reviewRightsDeclaration(input: {
    ...
    return db.transaction(async (tx) => {
      ...
      for (const manifestAsset of manifestAssets) {
        ...
        await lockAudioVersionCandidateModeration(tx, version.id);
  ...
  async function getPublicRelease(input: { releaseId: string }) {
    ...
    if ((await listRestrictedAudioVersionIds(db, recordings.flatMap(recording => recording.masterAssetVersionId
      ? [recording.masterAssetVersionId] : []))).size) return null;
    ...
    const publicRecordings = recordings.map((recording) => {
      ...
      const { masterAssetVersionId: _masterVersionId, ...publicRecording } = recording;
      return {
        ...publicRecording,
        creation,
        credits: credits.filter((credit) => credit.recordingId === recording.recordingId)
  ...
  return {
    ...
    listProjectAssets,
    getStorageUsage,
    preflightCollaboratorRevisionUpload,
