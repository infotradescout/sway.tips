        ...
        (
          ...
          + coalesce((
            ...
            where candidate.performer_id = ${input.performerId}
              ...
              and not exists (
                select 1 from audio_project_asset_versions accepted
                where accepted.upload_session_id = candidate.upload_session_id
                  and accepted.storage_provider = candidate.storage_provider
                  and accepted.storage_bucket = candidate.storage_bucket
                  and accepted.storage_key = candidate.storage_key
              )
          ), 0)
        )::text
