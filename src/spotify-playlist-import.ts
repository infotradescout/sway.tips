...
export async function importSpotifyPlaylistFromBrowser(options: ImportOptions): Promise<boolean> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const hint = Number.isSafeInteger(wait) && wait > 0 && wait <= 86_400 ? ` Retry after ${wait} seconds.` : '';
      throw new Error((typeof data?.error === 'string' ? data.error : 'Spotify import could not be confirmed. Refresh Sources before retrying.') + hint);
    }
