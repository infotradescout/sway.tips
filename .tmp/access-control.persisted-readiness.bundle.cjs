...
__export(access_control_exports, {
  ...
  routeFamilyGuard: () => routeFamilyGuard
});
...
__export(schema_exports, {
  ...
  activeRoomRegistry: () => activeRoomRegistry,
  auditEvents: () => auditEvents,
  captureModeEnum: () => captureModeEnum,
  clientPendingActions: () => clientPendingActions,
  gigAccessGrants: () => gigAccessGrants,
  gigSessionStatusEnum: () => gigSessionStatusEnum,
  gigSessions: () => gigSessions,
  ...
  payouts: () => payouts,
  pendingActionStatusEnum: () => pendingActionStatusEnum,
  performerMemberships: () => performerMemberships,
  performerOnboardingStatusEnum: () => performerOnboardingStatusEnum,
  performerSessions: () => performerSessions,
  performers: () => performers,
  refundStatusEnum: () => refundStatusEnum,
  requestBoosts: () => requestBoosts,
  requestStatusEnum: () => requestStatusEnum,
  requests: () => requests,
  userRoleEnum: () => userRoleEnum,
  users: () => users
});
var userRoleEnum = pgEnum("user_role", ["patron", "performer", "admin", "support"]);
var performerOnboardingStatusEnum = pgEnum("performer_onboarding_status", [
  "created",
  "profile_started",
  "gig_ready",
  "payments_limited",
...
var paymentAccountStatusEnum = pgEnum("payment_account_status", [
  "not_started",
  "created",
  "charges_enabled",
  "payouts_enabled",
...
var kycStatusEnum = pgEnum("kyc_status", [
  "not_required",
  "required",
  "submitted",
  "verified",
...
var gigSessionStatusEnum = pgEnum("gig_session_status", [
  "draft",
  "scheduled",
  "active",
  "closeout_pending",
...
var requestStatusEnum = pgEnum("request_status", [
  "submitted",
  "payment_pending",
  "payment_authorized",
  "held_for_review",
...
var captureModeEnum = pgEnum("capture_mode", ["automatic", "manual"]);
var refundStatusEnum = pgEnum("refund_status", ["not_refunded", "pending", "refunded", "failed"]);
var payoutStatusEnum = pgEnum("payout_status", ["not_started", "pending", "paid_out", "failed"]);
var moderationStatusEnum = pgEnum("moderation_status", ["allowed", "held_for_review", "blocked"]);
var pendingActionStatusEnum = pgEnum("pending_action_status", [
  "pending",
  "retrying",
  "reconciled",
  "expired",
...
var performers = pgTable("performers", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  verificationRequiredAtAmount: integer("verification_required_at_amount").notNull().default(1e4),
  ...timestamps
}, (table) => ({
...
var performerMemberships = pgTable("performer_memberships", {
  id: uuid("id").primaryKey().defaultRandom(),
...
var gigSessions = pgTable("gig_sessions", {
  id: uuid("id").primaryKey().defaultRandom(),
  performerId: uuid("performer_id").notNull().references(() => performers.id),
...
var gigAccessGrants = pgTable("gig_access_grants", {
  id: uuid("id").primaryKey().defaultRandom(),
...
var activeRoomRegistry = pgTable("active_room_registry", {
...
var performerSessions = pgTable("performer_sessions", {
  id: uuid("id").primaryKey().defaultRandom(),
...
var requestBoosts = pgTable("request_boosts", {
  id: uuid("id").primaryKey().defaultRandom(),
...
var moderationEvents = pgTable("moderation_events", {
  id: uuid("id").primaryKey().defaultRandom(),
  actorUserId: uuid("actor_user_id").references(() => users.id),
...
var activeBlocks = pgTable("active_blocks", {
  id: uuid("id").primaryKey().defaultRandom(),
...
var clientPendingActions = pgTable("client_pending_actions", {
  id: uuid("id").primaryKey().defaultRandom(),
  clientRequestId: text("client_request_id").notNull(),
  idempotencyKey: text("idempotency_key").notNull(),
  gigId: uuid("gig_id").notNull().references(() => gigSessions.id),
...
// src/db/client.ts
function createSwayDb(connectionString) {
...
var PERFORMER_SESSION_COOKIE_NAME = "sway_performer_session";
var DEFAULT_PERFORMER_SESSION_TTL_HOURS = 12;
...
function hashPerformerSessionToken(token) {
  return (0, import_node_crypto.createHash)("sha256").update(token, "utf8").digest("hex");
}
function readCookieHeaderValue(req, cookieName) {
...
function createPerformerSessionStore({
  databaseUrl,
  dbOverride,
  ...
  return {
    ...
    async issueSession({
      ...
      const expiresAt = new Date(now.getTime() + sessionTtlHours * 60 * 60 * 1e3);
      const [inserted] = await db.insert(performerSessions).values({
        actorUserId,
        tokenHash,
        expiresAt,
        revokedAt: null,
    ...
    async resolveSessionFromToken(token) {
      ...
      const [row] = await db.select({
        ...
        expiresAt: performerSessions.expiresAt
      }).from(performerSessions).where(and(
    ...
    async revokeSessionFromToken(token) {
      if (!db || !token) return null;
      const tokenHash = hashPerformerSessionToken(token);
      const now = /* @__PURE__ */ new Date();
      const [revoked] = await db.update(performerSessions).set({
        revokedAt: now
      }).where(and(
        eq(performerSessions.tokenHash, tokenHash),
        isNull(performerSessions.revokedAt)
...
function renderProtectedRouteRecovery(status, reason) {
...
function writeResolvedActor(req, actor) {
...
function hasResolvedActor(req) {
  return req.headers[HYDRATED_ACTOR_HEADER] === "1";
}
function resolveActor(req) {
  if (hasResolvedActor(req)) {
    return {
      actorId: typeof req.headers[RESOLVED_ACTOR_HEADER] === "string" && req.headers[RESOLVED_ACTOR_HEADER].length > 0 ? req.headers[RESOLVED_ACTOR_HEADER] : null,
      sessionId: typeof req.headers[RESOLVED_SESSION_HEADER] === "string" && req.headers[RESOLVED_SESSION_HEADER].length > 0 ? req.headers[RESOLVED_SESSION_HEADER] : null,
      patronDeviceIdHash: typeof req.headers[RESOLVED_DEVICE_HEADER] === "string" && req.headers[RESOLVED_DEVICE_HEADER].length > 0 ? req.headers[RESOLVED_DEVICE_HEADER] : null
    };
  }
  return {
    actorId: typeof req.headers["x-sway-actor-id"] === "string" ? req.headers["x-sway-actor-id"] : null,
...
function fallbackVerificationUnavailable() {
  return {
    allowed: false,
    status: 503,
...
function invalidFallbackAssertion() {
  return {
    allowed: false,
    status: 403,
...
var DEFAULT_FALLBACK_ASSERTION_MAX_AGE_MS = 5 * 60 * 1e3;
var RESOLVED_ACTOR_HEADER = "x-sway-resolved-actor-id";
var RESOLVED_SESSION_HEADER = "x-sway-resolved-session-id";
var RESOLVED_DEVICE_HEADER = "x-sway-resolved-device-id-hash";
var HYDRATED_ACTOR_HEADER = "x-sway-actor-hydrated";
function parseFallbackActorIds(rawValue) {
  if (!rawValue) return /* @__PURE__ */ new Set();
  return new Set(
    rawValue.split(/[,\s]+/).map((value) => value.trim()).filter((value) => UUID_PATTERN.test(value))
  );
}
function createFallbackAccessPolicy() {
  return {
...
function parseFallbackAssertionMaxAgeMs(rawValue) {
  const parsed = Number(rawValue);
  if (!Number.isFinite(parsed) || parsed <= 0) {
...
function createFallbackVerificationConfig() {
...
function hasConfiguredFallback(policy, role) {
...
function allowsFallbackRole(policy, actorId, role) {
...
function fallbackRoleResult(actor, role) {
...
function verifyFallbackAssertion(req, actor, role, config) {
  ...
  const timestamp2 = readHeaderValue(req, "x-sway-fallback-timestamp");
  const signature = readHeaderValue(req, "x-sway-fallback-signature");
  if (!assertedRole || !timestamp2 || !signature) {
    return invalidFallbackAssertion();
  }
  if (assertedRole !== role) {
    return invalidFallbackAssertion();
  }
  const assertedAt = Date.parse(timestamp2);
  if (!Number.isFinite(assertedAt) || Math.abs(Date.now() - assertedAt) > config.maxAgeMs) {
    return invalidFallbackAssertion();
  }
  const expectedSignature = (0, import_node_crypto2.createHmac)("sha256", config.signatureSecret).update(createFallbackAssertionPayload({
  ...
  if (signatureBuffer.length !== expectedBuffer.length || !(0, import_node_crypto2.timingSafeEqual)(signatureBuffer, expectedBuffer)) {
    return invalidFallbackAssertion();
  }
  return null;
}
function resolveTalentFallbackAccess(req, actor, policy, verificationConfig) {
...
function resolveAdminFallbackAccess(req, actor, policy, verificationConfig, options) {
  if (!actor.actorId) return missingActor();
...
async function hasAdminRole(db, actorId) {
  const rows = await db.select({ id: users.id }).from(users).where(and(eq(users.id, actorId), eq(users.role, "admin"))).limit(1);
  return rows.length > 0;
}
async function hasSupportRole(db, actorId) {
  const rows = await db.select({ id: users.id }).from(users).where(and(eq(users.id, actorId), eq(users.role, "support"))).limit(1);
  return rows.length > 0;
}
async function getActorRole(db, actorId) {
  const rows = await db.select({ role: users.role }).from(users).where(eq(users.id, actorId)).limit(1);
  if (!rows.length) {
    return null;
  }
  return rows[0].role;
}
async function hasTalentRole(db, actorId) {
  const ownerRows = await db.select({ id: performers.id }).from(performers).where(eq(performers.ownerUserId, actorId)).limit(1);
  if (ownerRows.length > 0) return true;
  const membershipRows = await db.select({ id: performerMemberships.id }).from(performerMemberships).where(eq(performerMemberships.userId, actorId)).limit(1);
  if (membershipRows.length > 0) return true;
  const accessRows = await db.select({ id: gigAccessGrants.id }).from(gigAccessGrants).where(eq(gigAccessGrants.userId, actorId)).limit(1);
...
function createAccessControl({
  ...
  const performerSessionStore = db ? performerSessionStoreOverride ?? createPerformerSessionStore({ databaseUrl, dbOverride: db }) : null;
  function actorFromSession(session, req) {
  ...
  async function hydrateRequestActor(req) {
    if (hasResolvedActor(req)) {
  ...
  return {
    ...
    async requireGigMutationAccess(req, gigId) {
      ...
      const performerOwnership = await db.select({ performerId: gigSessions.performerId }).from(gigSessions).where(and(
      ...
      const gigRows = await db.select({ performerId: gigSessions.performerId }).from(gigSessions).where(eq(gigSessions.id, gigId)).limit(1);
      if (!gigRows.length) {
      ...
      const performerRows = await db.select({ id: performers.id }).from(performers).where(and(eq(performers.id, gigPerformerId), eq(performers.ownerUserId, actor.actorId))).limit(1);
      if (performerRows.length > 0) {
        return { allowed: true, actor, role };
      }
      const memberRows = await db.select({ id: performerMemberships.id }).from(performerMemberships).where(and(
      ...
      const grantRows = await db.select({ id: gigAccessGrants.id }).from(gigAccessGrants).where(and(
...
function routeFamilyGuard(accessControl) {
  return async (req, res, next) => {
    const shell = req.headers["x-sway-shell"];
    const demoPreviewShellAllowed = process.env.NODE_ENV !== "production" && process.env.VITE_SWAY_DEMO_MODE === "true" && req.method === "GET" && (shell === "talent" || shell === "admin");
    ...
    const guard = shell === "talent" ? accessControl.requireTalentAccess : shell === "admin" ? accessControl.requireAdminOrSupportAccess : shell === "overlay" ? accessControl.allowPublicOverlayAccess : shell === "dev-sandbox" ? accessControl.requireDevSandboxAccess : accessControl.allowPublicPatronAccess;
    const result = await guard(req);
    if (result.allowed === false) {
      if (isBrowserHtmlRequest(req)) {
        res.status(result.status).set({ "Content-Type": "text/html; charset=utf-8" }).send(renderProtectedRouteRecovery(result.status, result.reason));
        return;
      }
