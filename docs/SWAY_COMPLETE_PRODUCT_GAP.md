...
| Store status, correction-request intake, failures, and takedowns | Fail-closed sandbox lifecycle implemented; correction requests are recorded but not transmitted or resubmitted; only `sway_sandbox` is registered | **Implemented, unverified:** no contracted provider execution or live producer/receiver evidence |
