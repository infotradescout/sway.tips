...
### Bounded private candidate returns

The implementation now supports one private candidate revision for an explicitly selected source version. Deployment remains off by default with `SWAY_AUDIO_COLLABORATOR_REVISION_UPLOAD_ENABLED=false`. Enabling that flag is not sufficient authority: an administrator must issue an auditable, current `private_collaboration` capability for the performer through `/api/admin/performers/:performerId/private-collaboration-capability`. Grant/revoke requests require the existing persisted administrator guard, a reason and an idempotency key. This enrollment does not grant public playback, distribution or money authority.

A creator chooses a connected account and a source file, then grants one candidate upload with a fixed byte ceiling and expiry. The collaborator receives no project-wide upload permission. Before hashing, the client checks current scope and available storage through a read-only preflight; actual initiation checks again and atomically reserves storage. Hashing reads bounded chunks with progress and cancellation. Provider initiation, parts, assembly and cleanup use persisted intent, leases and reconciliation so an ambiguous response does not imply completion. Technical verification seals an immutable private candidate separate from ordinary file versions.

The current performer owner, with active project management and upload authority, can accept, reject or block a returned candidate. Acceptance binds an immutable decision to one exact new private working version, preserves the source and collaborator provenance, and does not choose a recording master or publish a release. Same-intent retries return the saved decision; other terminal decisions conflict. Candidate-specific holds and blocks prevent acceptance and playback; only persisted administrators can clear moderation holds. This does not implement an account-wide block or public takedown process.

Private candidate playback uses the same authenticated, non-cacheable byte-range response as source listening, with scope and moderation rechecked after storage opens. Storage accounting counts an accepted candidate and its new working version once because they reference the same exact preserved object.

Evidence includes disposable database migration upgrades, scoped upload/read/decision integrations, provider recovery, exact HTTP range bytes and actual React component checks. Local embedded PostgreSQL and local private object storage are not standalone PostgreSQL concurrency or live R2 evidence. Native browser proof remains blocked in the current validation environment; this paragraph does not claim production activation.

## Universal Collaborator Inbox

Private source listening is available in the inbox for shared audio when the recipient already has source-download permission. The authenticated `/api/talent/audio/file-grants/:grantId/listen` route supports browser seeking through exact HTTP byte ranges from the sealed original. Every request checks the active selected-version grant and connection; review-only permission does not grant listening access. Responses are private and non-cacheable, and original files are never transcoded or replaced. Unsupported browser formats retain the source-download fallback. This is private collaboration playback, not public release playback or Sway.DIO streaming. New local HTTP, storage-adapter and durable authorization proofs cover this path; production/browser verification is tracked separately.
