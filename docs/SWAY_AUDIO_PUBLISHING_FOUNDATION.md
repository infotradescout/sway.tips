...
The current implementation includes private original-file storage and upload foundations, one-time file-pairing claims, explicit selected-version share grants, exact-original download, grant-scoped review actions, revocation, a universal account Collaborator Inbox, and private-candidate intake only behind the default-off `SWAY_AUDIO_COLLABORATOR_REVISION_UPLOAD_ENABLED` flag. The local Wave 5A and Wave 5B candidates are not general availability. Candidate promotion, accept/reject/disposition, candidate-specific moderation/reporting/blocking/takedown, DistroKid/DSP cutover, public playback, creator-deal execution, store delivery, sales, and royalties remain incomplete. Capability flags stay false until each remaining capability has durable implementation and production evidence.
...
- keep a collaborator's submitted candidate in a separate immutable private-review table; the current slice has no route that can replace the source, create a Catalog version, become requestable, enter a release, or reach a delivery provider;
...
- quarantine or reject a file that fails integrity or playable-media format checks, without describing technical media parsing as malware scanning or moderation;
...
Resumable upload sessions are idempotent. A session records expected size and digest, provider upload identity, part size, parts, expiry, and completion. Owner-session replay is bound to the authenticated actor and an exact request fingerprint; reuse of the raw key by another authorized actor or for changed upload intent fails before provider I/O. Replaying an exact part returns its original receipt; replaying different bytes for the same part fails. Completing the same owner session twice must not create two asset versions, and completing the same candidate session twice must not create two candidates.

Wave 5B records immutable provider-operation intent before multipart initiation, part upload, assembly, or cleanup. A due-operation worker reconciles provider-success/transaction-failure ambiguity, including a still-sessionless initiation. Provider calls receive a deadline signal below the operation lease, while a durable heartbeat extends the fence until even a provider that ignores cancellation actually settles. A request timeout or transient heartbeat-renewal failure keeps retrying the lease in a detached settlement path and becomes reconciliation-required only after the underlying call settles; it never becomes safe to repeat merely because the request returned. Cleanup intent is a durable cross-operation fence: it prevents later part or assembly dispatch, proves the exact provider identity absent, terminalizes the session's mutation operations, and only then releases reserved quota. Grant and connection revocation reserve session-bound and sessionless cleanup receipts inside the same database transaction before authority is revoked, closing the crash gap between revocation and cleanup scheduling. Revocation endpoints report cleanup as retry-pending or in progress while a provider lease or cleanup receipt remains unresolved; they do not describe deferred cleanup as complete.
...
### Private candidate intake foundation

Private candidate intake is disabled unless `SWAY_AUDIO_COLLABORATOR_REVISION_UPLOAD_ENABLED` is exactly `true`. Even then, the source performer must hold a current `private_collaboration` capability, the two accounts must retain an active connection, and a creator with current project access-management authority must issue a bounded exact-source upload grant with an explicit creator-approved maximum candidate byte count. One grant binds one multipart upload session and at most one immutable candidate; the grant expires within seven days and the performer capability, exact issuing project authority, connection, exact-file grant, source identity, and byte ceiling are rechecked at the relevant initiation, part, completion, and collaborator read/list boundaries.

The collaborator can upload only a technically parseable audio file whose complete stored bytes match the declared byte count and SHA-256 digest. Sway records the container, codec, duration, sample rate, bit depth when available, channels, verifier, and evidence in `audio_candidate_revisions`. This is technical validation only; it is not content moderation, rights review, malware scanning, or approval. The table is append-only and separate from `audio_project_asset_versions`. No candidate ID is accepted by request, release-manifest, delivery, public playback, royalty, or payment paths. There is intentionally no promotion or disposition route in this slice.

A candidate and its active upload reservation consume the creator's bounded working-storage bytes and working-file count. A candidate never receives release-manifest graduation, so release count remains unlimited without turning collaboration into free general storage. After upload authority ends, a current project access manager may still inspect the sealed candidate while the collaborator is denied. Caught validation and authority-revocation cleanup failures write `audio_object_cleanup_receipts` with the exact private object identity for the local retry worker; cleanup never deletes a sealed source version or candidate. The local Wave 5B candidate now reserves sessionless initiation intent and records fenced provider-operation attempts, reconciliation, terminal evidence, and quota transfer or cleanup resolution before provider-backed work is accepted. Deterministic barriers cover authority loss after successful provider initiation and assembly, revocation or expiry while a part is in flight, later-dispatch denial, cleanup retry, terminal operation evidence, and exact quota retention/release. An operating-system proof now force-kills separate workers after multipart initiation, part persistence, and final assembly, then proves fresh workers recover one session, one part receipt, one sealed candidate, and the exact original bytes from the local private object store. The named strict Wave 5B proof runs that process-kill recovery plus quota, idempotency, expiry, completion, provider-timeout, and heartbeat-renewal races against an attested standalone PostgreSQL server with independent backends. This remains an unshipped default-off candidate: the same full-request process-kill recovery against Cloudflare R2 and the deployment candidate is still a pre-enable blocker.
...
`/account/collaboration` is the authenticated Collaborator Inbox for any Sway account; it does not require a performer profile, Pro Mode, payout readiness, or an active live room. It lists the account's private connections, files shared with it, ordinary shares it created, and sealed private candidates for projects it currently manages. Every row carries explicit read-review and revoke permissions so replacement-manager candidate visibility cannot grant ordinary-share controls. Within an active ordinary grant, the UI and server enforce the selected download, comment/change-request, and approval permissions. Pairing does not add a Catalog or send control for an ordinary account. Either participant can remove a connection, and an authorized grant participant can revoke selected-file access. Review history is scoped to the selected grant rather than inferred from the asset version alone.
...
The original inbox slice reuses the existing connection, pairing-token, selected-file grant, review-event, and audit boundaries. It adds account-universal routing, authorization, and UI; it introduces no new schema or migration and no distribution fee, sale, royalty, payout, Stripe, or live-room money behavior. The later private-candidate foundation adds purpose-bound grants, private candidate persistence, cleanup receipts, account-universal upload UI, and exact route authorization. It adds no public playback, provider delivery, moderation workflow, disposition workflow, or candidate-promotion behavior.
...
5. Continue the broader immutable-version and derivative workflows. Selected-file download, grant-scoped review/approval, and revocation are implemented. Wave 5A private-candidate intake exists only behind its default-off flag; candidate promotion, moderation/reporting/takedown, accept/reject/disposition, and general-availability evidence remain incomplete.
