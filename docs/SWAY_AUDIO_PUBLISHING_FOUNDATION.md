...
The current implementation includes private original-file storage and upload foundations, one-time file-pairing claims, explicit selected-version share grants, exact-original download, grant-scoped review actions, revocation, a universal account Collaborator Inbox, and private-candidate intake only behind the default-off `SWAY_AUDIO_COLLABORATOR_REVISION_UPLOAD_ENABLED` flag. Wave 5A is not general availability. Candidate promotion, accept/reject/disposition, candidate-specific moderation/reporting/blocking/takedown, DistroKid/DSP cutover, public playback, creator-deal execution, store delivery, sales, and royalties remain incomplete. Capability flags stay false until each remaining capability has durable implementation and production evidence.
...
Resumable upload sessions are idempotent. A session records expected size and digest, provider upload identity, part size, parts, expiry, and completion. Replaying an exact part returns its original receipt; replaying different bytes for the same part fails. Completing the same owner session twice must not create two asset versions, and completing the same candidate session twice must not create two candidates.
...
A candidate and its active upload reservation consume the creator's bounded working-storage bytes and working-file count. A candidate never receives release-manifest graduation, so release count remains unlimited without turning collaboration into free general storage. After upload authority ends, a current project access manager may still inspect the sealed candidate while the collaborator is denied. Caught validation and authority-revocation cleanup failures write `audio_object_cleanup_receipts` with the exact private object identity for the local retry worker; cleanup never deletes a sealed source version or candidate. Provider-success/process-termination ambiguity and sessionless orphan accounting remain pre-enable blockers and are not covered by that guarantee.
