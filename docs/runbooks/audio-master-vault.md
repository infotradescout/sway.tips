...
`npm run test:wave5b` is the local Wave 5B provider-durability aggregate. It checks migration/schema consistency; runs the provider-operation contract and embedded-PostgreSQL invariant proof; verifies deterministic local and R2-adapter reconciliation; force-kills child workers after multipart initiation, part persistence, and final assembly; and reruns owner storage policy, ordinary file collaboration, and private candidate integration. Fresh recovery workers must preserve the killed execute attempt, acquire a reconciliation lease, create exactly one upload session, part receipt, and sealed private candidate, and reproduce the exact original bytes without duplicate provider intent. The provider-operation proof also covers immutable intent, exact subject keys, bounded leases, provider-call deadlines below the lease, heartbeat renewal until a late provider call actually settles, cross-operation cleanup fencing, atomic domain/result finalization, dead-letter resolution, retained quota until explicit cleanup resolution, actor/grant isolation, and cleanup absence evidence. The service proofs use deterministic barriers for authority loss, timeout, heartbeat-renewal failure, and revocation or expiry while provider work is in flight. The separately named real-Express HTTP proof asserts that revocation during sessionless provider initiation returns `202` with pending cleanup rather than false completion. The process-kill proof uses the local private object store and R2 adapter behavior is simulated locally; no Cloudflare request is made. This aggregate is local evidence only: it does not activate the candidate feature, push a branch, create a release, or attest a deployed build.
