...
Migrations 0040 through 0042 are forward-only. Their explicit legacy and pre-ceiling candidate-grant revocations are durable security evidence; disabling the flag or rolling application code does not and must not reactivate that authority. If collaboration is later reauthorized, a current project manager must issue a fresh bounded grant with fresh intent and a fresh idempotency key.
