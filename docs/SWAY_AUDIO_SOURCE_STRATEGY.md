...
The expanded file parsers and source-import chooser are wired into the existing Sources workspace. Each named music app owns its file-import identity, so same-named exports from different apps remain separate. These imports are fallback capabilities, not full account/player integrations. Exact candidate validation and release evidence remain separate from this architecture document.
