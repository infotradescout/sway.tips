...
- `SWAY_AUDIO_COLLABORATOR_REVISION_UPLOAD_ENABLED` defaults to disabled. Only the exact lowercase value `true` exposes private-candidate grant, multipart upload, completion, and playback routes. Enabling it does not bypass current `private_collaboration` performer capability, active connection, exact-file grant, expiry, quota, integrity, or audit checks. It does not create a release, replace a Catalog version, or authorize delivery.
