...
- Production exact-original audio storage requires `SWAY_AUDIO_STORAGE_PROVIDER=local_private_fs`, `SWAY_AUDIO_LOCAL_DURABLE_MOUNT=true`, an absolute `SWAY_AUDIO_LOCAL_MOUNT_PATH`, and `SWAY_AUDIO_LOCAL_OBJECT_DIR` at or below that mount. Startup must verify the mount against `/proc/self/mountinfo`; an environment flag alone is not durability evidence. See `docs/runbooks/audio-master-vault.md`.
