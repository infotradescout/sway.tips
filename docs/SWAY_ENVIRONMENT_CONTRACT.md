...
- `SWAY_AUDIO_COLLABORATOR_REVISION_UPLOAD_ENABLED` enables bounded private candidate routes only when exactly `true`; the default is disabled. Current persisted performer capability and exact selected-file authority remain required. This does not enable publication, DSP or money flows.
